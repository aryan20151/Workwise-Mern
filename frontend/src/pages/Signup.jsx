...
import { FiUser, FiMail, FiLock, FiAlertCircle, FiBriefcase, FiEye, FiEyeOff } from 'react-icons/fi';

const Signup = () => {
  const { user, signup, isClerkEnabled } = useAuth();
  const navigate = useNavigate();
  const { signUp } = isClerkEnabled ? useSignUp() : { signUp: null };
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-blue-50 via-slate-50 to-slate-50">
      <div className="max-w-md w-full space-y-6 bg-white p-8 rounded-2xl shadow-xl border border-slate-100">
        ...
        {/* Direct Google / Gmail Signup Button */}
        ...
        <div className="relative flex items-center justify-center">
          ...
          <span className="bg-white px-3 text-xs font-semibold uppercase tracking-wider text-slate-400 absolute">or register with email</span>
        </div>
